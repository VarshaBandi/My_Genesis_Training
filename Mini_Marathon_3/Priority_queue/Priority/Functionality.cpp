...
{
    data.push(std::make_shared<Ev2Wheeler>(12,VehicleType::REGULAR,234.0f,32,21, 500,ConnectedTechType::DIGITAL));
    data.push(std::make_shared<Ev2Wheeler>(23,VehicleType::REGULAR,340.0f,32,21, 500,ConnectedTechType::DIGITAL));
    data.push(std::make_shared<EvCar>(50,VehicleType::TRANSPORT,983.f,89,82,300.0f,ChargingType::AC));
    data.push(std::make_shared<Ev2Wheeler>(45,VehicleType::REGULAR,500.0f,32,21, 500,ConnectedTechType::DIGITAL));
    data.push(std::make_shared<EvCar>(38,VehicleType::TRANSPORT,383.f,21,38,199.0f,ChargingType::DC_FAST_CHARGE));
}
...
void DisplayObjects(MyPriorityQueue& data)
{
    while(!data.empty())
    {
        auto Ev2 = std::dynamic_pointer_cast<Ev2Wheeler>(data.top());
        if(Ev2!=nullptr)
        {
            std::cout<<*Ev2<<"\n";
            data.pop();
        ...
        {
            auto Ev = std::dynamic_pointer_cast<EvCar>(data.top());
            std::cout<<*Ev<<"\n";
            data.pop();
        }
    }
